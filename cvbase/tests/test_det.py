...
class TestDet(object):
    ...
    def test_bbox_transform(self):
        # test 1 simple test with 2 proposals and 1 gt
        proposals1 = np.array([[100, 120, 300, 240], [0, 20, 20, 100]])
        ...
        # test 2 should have the same ouput as test 1
        ...
        # test3 compare to naive bbox transform code
        ...
        # test 4 combine 3D data shape = (2,2,4)
        ...
        deltas1_gt_0 = np.array([[0.01243781, -0.28925619, -0.2534489, -0.08626036]])
        ...
        assert_array_almost_equal(deltas1_gt_0, deltas1_0)
        assert_array_almost_equal(deltas1_gt_1, deltas1_1)
    ...
    def test_bbox_transform_inv(self):
        ...
        #deltas1_0 = bbox_transform(proposals1, gt1_0)
        #deltas1_1 = bbox_transform(proposals1, gt1_1)
        deltas1_0 = np.array([[0.01243781, -0.28925619, -0.2534489, -0.08626036], [ 0.79268295, 0.13245033, 1.33628392, -0.30774966]])
        deltas1_1 = np.array([[0.24875621, -0.57851237, -0.68818444, 0.], [1.95121956, -0.09933775, 0.90154845, -0.22148931]])
        ...
        gt1_0 = np.array([[[125, 90, 280, 200]]])
        gt1_1 = np.array([[[200, 50, 300, 170]]])
        gt1 = np.array([[np.hstack((gt1_0[0][0], gt1_1[0][0])), np.hstack((gt1_0[0][0], gt1_1[0][0]))]])
        #deltas1_0 = bbox_transform(proposals1, gt1_0)
        #deltas1_1 = bbox_transform(proposals1, gt1_1)
        deltas1_0 = np.array([[[0.01243781, -0.28925619, -0.2534489, -0.08626036], [ 0.79268295, 0.13245033, 1.33628392, -0.30774966]]])
        deltas1_1 = np.array([[[0.24875621, -0.57851237, -0.68818444, 0.], [1.95121956, -0.09933775, 0.90154845, -0.22148931]]])
        deltas1 = np.array([np.hstack((deltas1_0[0][0], deltas1_1[0][0])), np.hstack((deltas1_0[0][1], deltas1_1[0][1]))])
        assert_array_almost_equal(bbox_transform_inv(proposals1, deltas1), gt1.astype(np.float32), decimal = 5)
